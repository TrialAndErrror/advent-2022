...
impl SymbolManager9001 {
    pub fn from_data(data: Vec<&str>) -> SymbolManager9001 {
        println!("Making Symbol Manager...");
    ...
    fn process_instruction(&mut self, instruction: String) {
        ...
        let source_row = info[3].parse::<usize>().unwrap() - 1;
        let count_to_move: usize = {
            let written_count = info[1].parse::<usize>().unwrap();
            if written_count > self.data[source_row].len() {
                self.data[source_row].len()
            } else {
                written_count
            }
        };
        ...
        self.data[dest_row].extend(v[split_index..].to_vec());
    }
